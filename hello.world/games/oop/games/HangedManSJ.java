...
public class HangedManSJ{
	...
	/*
	 * Une fonction qui joue une partie et offre aux joueurs la possibilité de
	 * continuer à jouer ou non. La fonction qui choisit le mot à deviner parmi la
	 * liste des mots possibles, en utilisant un nombre généré aléatoirement.
	 * Regardez la classe Random dans le package java.util.
	 */
	...
	/*
	 * The function initialize the game for the chosen word, given by its index in
	 * the array of words.
	 */
	...
	public static void playGame(InputStream is, PrintStream ps, Scanner scan_air) {
		...
		while (la_vida > 0 && !guessed()) {
			...
			String entree = scan_air.nextLine();
			
			
			
			while (entree.isEmpty() || entree.length() != 1 || 
				  (entree.charAt(0) < 'a' || entree.charAt(0) > 'z') && 
				  (entree.charAt(0) < 'A' || entree.charAt(0) > 'Z')) {
				ps.print("ERREUR : il me faut une lettre valide : ");
				entree = scan_air.nextLine();
			}
